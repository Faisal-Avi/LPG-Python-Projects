...
zk = ZK('192.168.188.6', port=4370, timeout=5, password=0, force_udp=False, ommit_ping=False)
conn = zk.connect()
print(conn)
...
while True:
    try:
        ...
        for tpl in attendance_list:
            ...
            print(t_hour)
            if l_user_id != '':
                cur_oracle.callproc("push_attendance_date", [l_attendance_date,l_user_id,l_attendance_status,l_unit_no])
                con.commit()
                print(l_user_id)
